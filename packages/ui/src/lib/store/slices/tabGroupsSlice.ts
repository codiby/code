...
export interface TabGroupsSlice {
  ...
  /** Manual folder order, per parent (`''` = root). Persisted server-side as
   *  `groupOrder` so every window and device lists folders the same way. */
  ...
  setGroupOrder: Dispatch<SetStateAction<Record<string, string[]>>>;
...
export const createTabGroupsSlice: SliceCreator<TabGroupsSlice> = (set) => ({
  ...
  setGroupOrder: (u) => set(s => ({ groupOrder: apply(s.groupOrder, u) })),
