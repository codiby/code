...
interface Props {
   ...
   *  new checkout so the caller can spawn a session there. */
  onCreated: (path: string) => void;
}

/**
 * Desktop worktree creation. Centered-card dialog over a dimmed
 * backdrop; mirrors the inline worktree form inside `NewSessionModal`
 * but as a standalone modal. The full-screen mobile variant lives in
      ...
      >
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          >
            {logs.length === 0 && status === 'idle' && (
              <div className="text-zinc-600">Configure the worktree above and click <em>Create</em> to start.</div>
            )}
        ...
        <div className="px-5 py-3 border-t border-border flex items-center justify-end gap-2 shrink-0">
          ...
          {status === 'done' && resultPath ? (
            <Button size="sm" onPress={() => { onCreated(resultPath); onClose(); }}>
              Use it
            </Button>
          ) : (
